...
# from io import SegmentedObject
...
class MOTSResults:
  def __init__(self):
    ...
    self.fp = 0  # number of false positives 误报
    self.fn = 0  # number of false negatives 漏报
    self.MOTSA = 0
    self.sMOTSA = 0
    self.MOTSP = 0
    self.MOTSAL = 0
...
def compute_MOTS_metrics_per_sequence(seq_name, gt_seq, results_seq, max_frames, class_id,
  ...
  for gt in gt_seq:
    gt_frame.append(gt)
  # Iterate over frames in this sequence  遍历序列的所有帧
  ...
  for f in gt_frame:
    ...
    if f in gt_seq:
      for obj in gt_seq[f]:
        if obj.class_id == ignore_class:
          ...
          g.append(obj)
          gt_track_ids.add(obj.track_id) # 像素点
    if f in results_seq:
      for obj in results_seq[f]:
        if obj.class_id == class_id:
          t.append(obj)
          tr_track_ids.add(obj.track_id) #像素点 两者存在区别
    ...
    results_obj.n_gt += len(g) # 开始为0 + 当前帧gt的数目
    results_obj.n_tr += len(t) # 开始为0 +
    ...
    tmpfp = 0
    tmpfn = 0
    tmpc = 0  # this will sum up the overlaps for all true positives  所有匹配的mask-iou和
    tmpcs = [0] * len(g)  # this will save the overlaps for all true positives 保存所有匹配的overlaps 
    ...
    #   seq_trajectories  dict(像素点:-1,)
    #   tracks_valid  ---  track 的数目
    num_associations = 0
    # seq_trajectories   defaultdict(<class 'list'>, {215: [-1], 235: [-1], 255: [-1]})
    for row, gg in enumerate(g):
      for col, tt in enumerate(t):
        c = overlap_function(gg, tt) # 计算mask iou      
        if c > 0.5:
          ...
          num_associations += 1
    '''
      seq_trajectories  defaultdict(<class 'list'>, {215: [29], 235: [149], 255: [76]}) key--gt像素点  value--track的像素点
      tmpcs [0.9554180548944947, 0.943082577469063, 0.9526243145447084] 匹配上的mask-iou
      tmptp = 3 匹配上的数目
    '''
  ...
  if len(seq_trajectories) != 0:
    for g in seq_trajectories.values():
      ...
      last_id = g[0]
      # first detection (necessary to be in gt_trajectories) is always tracked 第一帧跟踪
